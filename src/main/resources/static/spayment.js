...
function initialize() {
    ...
    cmbSupplier.addEventListener("change", getgrnbysupplier);
    cmbGrnId.addEventListener("change", getgrnamount);
...
function paginate(page) {
    ...
    if (oldspayment == null) {
        ...
        if (getErrors() == '' && getUpdates() == '') {
            ...
            paginate = window.confirm("Form has Some Errors or Update Values. " + "Are you sure to discard that changes ?");
        }
    }
    if (paginate) {
        activepage = page;
...
function btnPrintRowMC() {
    ...
    newwindow.document.write("<html>" + "<head><style type='text/css'>.google-visualization-table-th {text-align: left;}</style>" + " <link rel='stylesheet' href='resources/bootstrap/css/bootstrap.min.css'></head>" + "<body><div>" + format + "</div>" + "<script>printformtable.removeAttribute('style')</script>" + "</body></html>");
    setTimeout(function () {
        newwindow.print();
        newwindow.close();
    }, 1500);
...
function disableButtons(add, upd, del) {
    ...
    for (index in spayments) {
        if (spayments[index].paymentstatus_id.name == "Deleted") {
            ...
            tblSpayment.children[1].children[index].lastChild.children[0].disabled = true;
            tblSpayment.children[1].children[index].lastChild.children[0].style.cursor = "not-allowed";
...
function btnAddMC() {
    if (getErrors() == "") {
        if (txtCDate.value == "" || txtchequeno.value == "" || txtBAccNo.value == "" || txtBACCname.value == "" || txtBname.value == "" || txtBBName.value == "" || dteDepodate.value == "" || txtTId.value == "") {
            swal({
...
function getgrnbysupplier() {
    grnbysupplierlist = httpRequest("/grn/grnbysupplier?supplierid=" + JSON.parse(cmbSupplier.value).id, "GET");
    fillCombo(cmbGrnId, "Select GRN", grnbysupplierlist, "grncode", "");
}

function getgrnamount() {

    txtgrnAmount.value = parseFloat(JSON.parse(cmbGrnId.value).totalamount).toFixed(2);
    spayment.grnamount = txtgrnAmount.value;
    txtgrnAmount.style.border = valid;

    txtTAmount.value = parseFloat(JSON.parse(cmbGrnId.value).nettotal).toFixed(2);
    spayment.nettotal = txtTAmount.value;
    txtTAmount.style.border = valid;
...
function txtPAmountCH() {
    txtBAmount.value = (parseFloat(txtPAmount.value).toFixed(2) - parseFloat(txtTAmount.value).toFixed(2)).toFixed(2);
    spayment.balanceamount = parseFloat(txtBAmount.value).toFixed(2);
    txtBAmount.style.border = valid;

    if (oldspayment != null && spayment.paidamount != oldspayment.paidamount) {
        txtPAmount.style.border = updated;
    }

    if (oldspayment != null && spayment.balanceamount != oldspayment.balanceamount) {
...
function filldata(spay) {
    ...
    if (spayment.chequedate == null) txtCDate.style.border = initial;

    if (spayment.chequeno == null) txtchequeno.style.border = initial;

    if (spayment.bankaccno == null) txtBAccNo.style.border = initial;

    if (spayment.bankaccname == null) txtBACCname.style.border = initial;

    if (spayment.depositeddatetime == null) dteDepodate.style.border = initial;

    if (spayment.bankname == null) txtBname.style.border = initial;

    if (spayment.bankbranchname == null) txtBBName.style.border = initial;

    if (spayment.transferid == null) txtTId.style.border = initial;
}
...
function btnPrintTableMC(spay) {
    ...
    newwindow.document.write("" + "<html>" + "<head><style type='text/css'>.google-visualization-table-th {text-align: left;} .modifybutton{display: none} .isort{display: none}</style>" + "<link rel='stylesheet' href='resources/bootstrap/css/bootstrap.min.css'/></head>" + "<body><div style='margin-top: 150px; '> <h1>Supplier payments Details : </h1></div>" + "<div>" + formattab + "</div>" + "</body>" + "</html>");
    setTimeout(function () {
        newwindow.print();
        newwindow.close();
    }, 1500);
